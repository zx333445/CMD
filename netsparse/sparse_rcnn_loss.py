...
from .model_utils import reduce_sum, get_gpu_num_solo
...
class HungarianMatcher(object):
    ...
    @torch.no_grad()
    def __call__(self, predicts_cls, predicts_box, gt_boxes, gt_labels, shape_norm):
        """
        :param predicts_box: [bs,proposal,4]
        :param predicts_cls: [bs,proposal,2]
        ...
        combine_labels = torch.cat(gt_labels)
        ...
        cls_cost = positive_loss[:, combine_labels.long()] - negative_loss[:, combine_labels.long()]

        pred_norm = predicts_box / shape_norm[None, :]
        target_norm = combine_gts / shape_norm[None, :]
        
        l1_cost = torch.cdist(pred_norm, target_norm, p=1)

        pred_expand = predicts_box[:, None, :].repeat(1, len(combine_gts), 1).view(-1, 4)
        target_expand = combine_gts[None, :, :].repeat(len(predicts_box), 1, 1).view(-1, 4)
...
class SparseRCNNLoss(object):
    ...
    def __call__(self, cls_predicts, reg_predicts, targets, shape):
        ...
        pos_num = sum([target['boxes'].shape[0] for target in targets])
        ...
        for imme_idx in range(len(cls_predicts)):
            matches = self.matcher(cls_predicts[imme_idx].detach(), reg_predicts[imme_idx].detach(), gt_boxes, gt_labels, shape_norm)

            match_cls_bidx = sum([[i] * len(j) for i, j, _ in matches], [])
            match_proposal_idx = sum([j for _, j, _ in matches], [])
            match_cls_label_idx = torch.cat([gt_labels[i][k].long() for i, _, k in matches])
            match_box = torch.cat([gt_boxes[i][k] for i, _, k in matches])
        ...
        l1_loss = self.l1_weights * torch.nn.functional.l1_loss(box_pred / shape_norm[None, :],
                                                                ...
                                                                reduction="none").sum()
        pos_num = torch.tensor(pos_num, device=cls_predicts.device).item()
