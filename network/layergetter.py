...
class ConvNextLayerGetter(nn.Module):
    ''''''
    def __init__(self, model):
        super().__init__()
        self.model = model
    ...
    def forward(self,x):
        x = self.model.features.get_submodule('0')(x)
        C2 = x = self.model.features.get_submodule('1')(x)
        self.out_dict["0"] = C2
        x = self.model.features.get_submodule('2')(x)
        C3 = x = self.model.features.get_submodule('3')(x)
        self.out_dict["1"] = C3
        x = self.model.features.get_submodule('4')(x)
        C4 = x = self.model.features.get_submodule('5')(x)
        self.out_dict["2"] = C4
        x = self.model.features.get_submodule('6')(x)
        C5 = x = self.model.features.get_submodule('7')(x)
        self.out_dict["3"] = C5
        return self.out_dict



class EfficietnetLayerGetter(nn.Module):
    ''''''
    def __init__(self, model):
        super().__init__()
        self.model = model
