...
def _default_anchorgen():
    anchor_sizes = tuple((x, int(x * 2 ** (1.0 / 3)), int(x * 2 ** (2.0 / 3))) for x in [32, 64, 96, 128, 256])
    ...
    def compute_loss(self, targets, head_outputs, matched_idxs):
        ...
        for targets_per_image, cls_logits_per_image, matched_idxs_per_image in zip(targets, cls_logits, matched_idxs):
            ...
            losses.append(
                ...
                / max(1, num_foreground) # type: ignore
            )
...
class RetinaNetRegressionHead(nn.Module):
    ...
    def __init__(self, in_channels, num_anchors, norm_layer: Optional[Callable[..., nn.Module]] = None):
        ...
        torch.nn.init.zeros_(self.bbox_reg.bias) # type: ignore
